...
class Handler(BaseHTTPRequestHandler):
    ...
    def _send_svg(self, path):
        # only serve files from the local svg/ directory, no path traversal
        sub = path[len("/svg/"):]
        if not sub or ".." in sub or "/" in sub:
            self.send_error(404)
            return
        fpath = os.path.join(HERE, "svg", sub)
        if not os.path.isfile(fpath):
            self.send_error(404)
            return
        self._send_file(fpath, "image/svg+xml")
